...
public class DataConnection {
     ...
     * 
     */
    public DataConnection(String filename){
    ...
    public ResultSet getRecordCount(String dbName){
        ...
        try{
            rs = this.s.executeQuery(command);
            //rs = s.getResultSet();
    ...
    /**
     * Close connection and statements to database
     */
    ...
    public ResultSet getJobList(int year,int weekNumber){
        ...
        try{
        stt = this.conn.createStatement();
        }catch(Exception e){
            connectionErr(e);
        }
        String command =  "SELECT * FROM TIMEDATA WHERE ";

        //select job numbers starting 2 weeks prior to date to current week.
        for(int i=0;i<4;i++){
            command+="WEEK_NUMBER LIKE '" + Integer.toString(weekSearch.get(Calendar.WEEK_OF_YEAR)) +
                "' AND YEAR LIKE '" + Integer.toString(weekSearch.get(Calendar.YEAR)) +"'" + " OR ";
            weekSearch.add(Calendar.WEEK_OF_YEAR,1);
        }

        //Add one week ahead of current week.
        command+="WEEK_NUMBER LIKE '" + Integer.toString(weekSearch.get(Calendar.WEEK_OF_YEAR)) +
                "' AND YEAR LIKE '" + Integer.toString(weekSearch.get(Calendar.YEAR))+"'";
    ...
    public boolean createTimeKeeperTables(){
     ...
     public boolean createProjectTables(){
        Boolean success = false;

        String projIDtbl = "CREATE TABLE Projects (" +
                "ID AUTOINCREMENT CONSTRAINT PKEY Primary Key, " +
                "LocalJob VARCHAR(25), " +
                "Project VARCHAR(25), " +
                "Customer VARCHAR(30), " +
        ...
        String taskTbl = "CREATE TABLE ProjectTasks (" +
                "ID AUTOINCREMENT CONSTRAINT PKEY Primary Key, " +
                "ProjectID INTEGER, " +
                "TaskNum VARCHAR(15), " +
                "TaskDescription VARCHAR(150) )";

        String stateTbl = "CREATE TABLE States (" +
                "ID AUTOINCREMENT CONSTRAINT PKEY Primary Key, " +
                "State VARCHAR(25) )";

        String cityTbl = "CREATE TABLE Cities (" +
                "ID AUTOINCREMENT CONSTRAINT PKEY Primary Key, " +
                "State VARCHAR(15), " +
                "City VARCHAR(30) " +
                ")";

        String distrTbl = "CREATE TABLE Distributers (" +
                "ID AUTOINCREMENT CONSTRAINT PKEY Primary Key, " +
                "Distributer VARCHAR(30)) ";

        String custTbl = "CREATE TABLE Customers (" +
                "ID AUTOINCREMENT CONSTRAINT PKEY Primary Key, " +
                "Customer VARCHAR(30), " +
                "City VARCHAR(30), " +
                "State VARCHAR(15))";
        ...
        try{
            this.s.executeUpdate(projIDtbl);
            this.s.executeUpdate(taskTbl);
            this.s.executeUpdate(stateTbl);
            this.s.executeUpdate(cityTbl);
            this.s.executeUpdate(distrTbl);
            this.s.executeUpdate(custTbl);
            success = true;
        }
            catch (Exception e) {
                connectionErr(e);
    ...
    public boolean populateUSstates(){
        ...
        String[] states = new String[]
        {
         ...
         try{
            ...
            {
                this.ps = this.conn.prepareStatement("INSERT INTO States " +
                    "(State) VALUES (?)");
                this.ps.setString(1,states[i]);
                this.ps.executeUpdate();
             }
            this.ps.close();
            sucess = true;
    ...
    {
        /**
         * check if there are any existing logs for the
    ...
    public String getState(int ID)
    {
        String stateName="";
        ResultSet rs = null;
        String command ="SELECT * FROM States WHERE ID = " + Integer.toString(ID);
    ...
    public java.util.ArrayList<String[]> getCustomerList()
    {
        java.util.ArrayList<String[]> customers = new java.util.ArrayList<String[]>();
        /*
         * Customer Object decription:
         *          Object =  CustomerID, Customer Name,
         *                    Customer City, Customer State Text
         */
        ResultSet rs = null;
        ...
        try{
            ...
            while(rs.next())
            {
                ...
                customers.add(new String[]{Integer.toString(ID),cust,city,state});
            }
        }
        catch (Exception e) {
            connectionErr(e);
    ...
    public java.util.ArrayList<String> getStateList()
    {
        java.util.ArrayList<String> states = new java.util.ArrayList<String>();

        ResultSet rs = null;
        String command ="SELECT * FROM States";
    ...
    public java.util.ArrayList<String> getCityList()
    {
        ...
        public java.util.ArrayList<String> getDistributerList()
    {
        java.util.ArrayList<String> dist = new java.util.ArrayList<String>();

        ResultSet rs = null;
        String command ="SELECT * FROM Distributers";
        ...
        try{
            ...
            while(rs.next())
            {
                dist.add(rs.getString("Distributer"));
            }
        }
        catch (Exception e) {
            connectionErr(e);
    ...
    public boolean addProject(String[] project)
    {
        boolean sucess = false;
        try{
            this.ps = this.conn.prepareStatement("INSERT INTO Projects " +
                "(LocalJob, Project, Customer, Distributer, City, State) " +
                "values(?,?,?,?,?,?)");
            this.ps.setString(1, project[0]);
            this.ps.setString(2, project[1]);
            this.ps.setString(3, project[2]);
            this.ps.setString(4, project[3]);
            this.ps.setString(5, project[4]);
            this.ps.setString(6, project[5]);
            this.ps.executeUpdate();
            this.ps.close();
            sucess = true;
        }
    ...
    public Object[] getJobDetails(String project)
    {
        Object[] results = new Object[7];
        results[0] = false;
        ResultSet rs = null;

        String command ="SELECT * FROM Projects WHERE (Project = \'" + project + "\' )";
        ...
        try{
            ...
            {
                results[0] = true;
                results[1] = rs.getInt("ID");
                results[2] = rs.getString("LocalJob");
                results[3] = rs.getString("Customer");
                results[4] = rs.getString("City");
                results[5] = rs.getString("State");
                results[6] = rs.getString("Distributer");
            }
        }
            catch (Exception e) {
            connectionErr(e);
    ...
    public void updateProject(int ID, String[] record)
    {
        try{
            this.ps = this.conn.prepareStatement("UPDATE Projects SET " +
                "LocalJob = ?, Customer = ?, Distributer = ?, City = ?, State = ? " +
                "WHERE(ID = ?)");
            this.ps.setString(1, (String)record[0]);
            this.ps.setString(2, (String)record[2]);
            this.ps.setString(3, (String)record[3]);
            this.ps.setString(4, (String)record[4]);
            this.ps.setString(5, (String)record[5]);
            this.ps.setInt(6, ID );
            this.ps.executeUpdate();
            this.ps.close();
    ...
    public void addDistributer(String dist)
    {
        ResultSet rs = null;
        String command ="SELECT * FROM Distributers WHERE (Distributer = \'" + dist + "\' )";
        ...
        try{
            ...
            {
                //new distributer
                this.ps = this.conn.prepareStatement("INSERT INTO Distributers " +
                "(Distributer) " +
                "values(?)");
                this.ps.setString(1, dist);
                this.ps.executeUpdate();
                this.ps.close();
            }
        }
    ...
    {
        ResultSet rs = null;
        String command ="SELECT * FROM Cities WHERE (City = \'" + city + "\'  AND " +
                "State = \'" + state + "\' )";
        ...
        try{
            ...
            {
                //new distributer
                this.ps = this.conn.prepareStatement("INSERT INTO Cities " +
                "(City,State) " +
                "values(?,?)");
                this.ps.setString(1, city);
                this.ps.setString(2, state);
                this.ps.executeUpdate();
                this.ps.close();
            }
        }
    ...
    {
        ResultSet rs = null;
        String command ="SELECT * FROM Customers WHERE (Customer = \'" + data[2] + "\'  AND " +
                "State = \'" + data[4] + "\' AND City = \'" + data[5] + "\' )";
        ...
        try{
            ...
            {
                //new distributer
                this.ps = this.conn.prepareStatement("INSERT INTO Customers " +
                "(Customer,City,State) " +
                "values(?,?,?)");
                this.ps.setString(1, data[2]);
                this.ps.setString(2, data[4]);
                this.ps.setString(3, data[5]);
                this.ps.executeUpdate();
                this.ps.close();
            }
        }
    ...
    public java.util.ArrayList<String> customerSearch(String[] data)
    {
        java.util.ArrayList<String> results = new java.util.ArrayList<String>();
        ResultSet rs = null;

        String command ="SELECT * FROM Projects WHERE ((Customer = \'" + data[0] + "\')";

        for(int i=1;i<data.length;i++)
        {
            command+= " OR ";
            command+="(Customer = \'" + data[i] + "\')";
        }
        ...
        try{
            ...
            while(rs.next())
            {
                results.add(rs.getString("Project"));
            }
        }
            catch (Exception e) {
            connectionErr(e);
